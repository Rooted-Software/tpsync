...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const anEvents = await getAnedotEvents(user.teamId, parseInt(skip), take)
    ...
    const event = await db.anedotEvent.findFirst({
      ...
      if (event) {
        ...
        try {
          ...
          if (res.status === 200) {
            updateAnedotEvent(event.id, true, 'success', queryObj.meta, queryObj.query)
           } else { 
            queryObj.meta.attentionString += 'virtuous error: ' + transData
            updateAnedotEvent(event.id, false, 'failure', queryObj.meta, queryObj.query)
           }
        ...
        updateAnedotEvent(event.id, false, 'tested', queryObj.meta, queryObj.query)
        return new Response(JSON.stringify(queryObj.toString()), {status: 200})
      }
