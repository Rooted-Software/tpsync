...
export function  MappingCreateButton({
  ...
  return (<div>
     {projects?.length ? (
          <select name="virtuousProjects" id="virtuousProjects" onChange={(e)=>setVirProjectID(e.target.value)} value={virProjectID}>
       
            {projects.map((project) => (
              <option key={project.project_id} value={project.project_id}>{project.onlineDisplayName ? project.onlineDisplayName :  project.name}</option>
            ))}
          ...
          <EmptyPlaceholder>
            ...
            <EmptyPlaceholder.Title>No Virtuous Projects Found</EmptyPlaceholder.Title>
            <EmptyPlaceholder.Description>
              You don&apos;t have any Virtuous projects yet. Start creating content.
            </EmptyPlaceholder.Description>
        ...
        {feProjects?.length ? (
          <select name="feProjects" id="feProjects" onChange={(e)=>setFeProjectID(e.target.value)} value={feProjectID}>
       
            {feProjects.map((feProject) => (
              <option key={feProject.id} value={feProject.id}>{feProject.description ? feProject.description : feProject.ui_project_id }</option>
            ))}
