...
export function KeygenButton({
  ...
  async function onClick() {
    ...
    const ckey = await window.crypto.subtle.importKey('jwk', jwk, { name: 'RSA-OAEP', hash: 'SHA-256' }, true, ["encrypt"])
    const encoded = await window.crypto.subtle.encrypt({ name: "RSA-OAEP",}, ckey, enc.encode(rawText));
